...
async def test_node_registration_and_health(client):
    ...
    with patch("kosatka_master.services.providers.agent_provider.AgentNodeProvider.sync_node") as mock_sync:
...
async def test_node_get_detail(client):
    headers = {"X-Kosatka-Key": settings.api_key}
    resp = await client.post("/api/v1/nodes/", json={"name": "detail-node", "address": "addr"}, headers=headers)
...
async def test_node_delete(client):
    headers = {"X-Kosatka-Key": settings.api_key}
    # Create first
    resp = await client.post("/api/v1/nodes/", json={"name": "to-delete", "address": "addr"}, headers=headers)
    node_id = resp.json()["id"]
...
async def test_node_registration_upsert(client):
    ...
    response = await client.post("/api/v1/nodes/", json={"name": "upsert", "address": "a2", "api_key": "newkey"}, headers=headers)
    assert response.status_code == 200
...
async def test_node_errors(client):
    ...
    response = await client.post("/api/v1/nodes/", json={"name": "dup", "address": "a2"}, headers=headers)
    assert response.status_code == 200
    assert response.json()["address"] == "a2"
