...
def test_next_free_address():
    ...
    state.peers["1"] = _wgcore.Peer(
        client_id="1", private_key="", public_key="", preshared_key="", address="10.8.0.2/32"
    )
...
async def test_bootstrap_server(tmp_path):
    ...
    with (
        patch("kosatka_agent.bootstrap.bootstrap_provider", AsyncMock()),
        patch(
            "kosatka_agent.providers._wgcore.generate_keypair",
            AsyncMock(return_value=("priv", "pub")),
        ),
