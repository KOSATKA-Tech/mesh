...
async def test_provision_requires_subscription(client, db_session):
    ...
    await client.post(
        "/api/v1/nodes/",
        json={"name": "n3", "address": "http://a3", "provider_type": "wireguard"},
        headers=headers,
    )
    ...
    response = await client.post(
        ...
        json={"external_id": "c3", "protocol": "wireguard"},
        headers=headers,
    )
    ...
    await client.post(
        ...
        json={"client_id": db_client.id, "plan_name": "Monthly", "expires_at": future.isoformat()},
        headers=headers,
    )
