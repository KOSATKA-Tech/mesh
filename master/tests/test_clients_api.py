...
async def test_provision_client(client, db_session):
    ...
    await client.post("/api/v1/nodes/", json={"name": "n1", "address": "http://a1", "provider_type": "wireguard"}, headers=headers)
    ...
    with patch("kosatka_master.api.v1.clients._call_agent") as mock_call:
        ...
        response = await client.post("/api/v1/clients/provision/", json={"external_id": "c1", "protocol": "wireguard"}, headers=headers)
...
async def test_provision_with_pin(client):
    ...
    resp = await client.post("/api/v1/nodes/", json={"name": "pin-node", "address": "http://pin", "provider_type": "xray"}, headers=headers)
    ...
    with patch("kosatka_master.api.v1.clients._call_agent") as mock_call:
        ...
        response = await client.post("/api/v1/clients/provision/", json={
            "external_id": "c1", 
            "protocol": "xray",
            "node_id": node_id
        }, headers=headers)
...
async def test_get_client_config_edge_cases(client):
    ...
    # 2. Node inactive
    await client.post("/api/v1/nodes/", json={"name": "inactive", "address": "addr"}, headers=headers)
    ...
    from datetime import datetime, timedelta, timezone
    future = datetime.now(timezone.utc).replace(tzinfo=None) + timedelta(days=30)
    await client.post("/api/v1/subscriptions/", json={"client_id": cid, "plan_name": "T", "expires_at": future.isoformat()}, headers=headers)
...
async def test_get_client_config(client):
    ...
    await client.post("/api/v1/nodes/", json={"name": "n2", "address": "http://a2", "provider_type": "wireguard"}, headers=headers)
...
async def test_provision_errors(client):
    ...
    # No nodes available
    response = await client.post("/api/v1/clients/provision/", json={"external_id": "c2", "protocol": "non-existent"}, headers=headers)
    assert response.status_code == 503
