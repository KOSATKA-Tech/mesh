...
async def test_install_wireguard():
    with patch("shutil.which", side_effect=[None, "/usr/bin/wg"]), \
...
async def test_install_amneziawg():
    with patch("shutil.which", return_value=None), \
         ...
         patch("httpx.AsyncClient.get", AsyncMock(return_value=MagicMock(status_code=200, content=b"bin"))), \
         patch("os.chmod"), \
...
async def test_install_marzban():
    with patch("shutil.which", side_effect=[None, None]), \
         ...
         patch("httpx.AsyncClient.get", AsyncMock(return_value=MagicMock(status_code=200, content=b"script"))), \
         patch("builtins.open", MagicMock()):
